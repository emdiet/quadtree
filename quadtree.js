export class Rectangle {
    constructor(x1, y1, x2, y2, reference) {
...
export default class QuadTree {
    constructor(maxObjects = 10, maxLevels = 4, level = 0, bounds = new Rectangle(-Infinity, -Infinity, Infinity, Infinity)) {
        ...
        this.children = [];
        this.objectCount = 0;
        this.coalescenceCount = this.maxObjects / 2;
    }
    overlaps(rect) {
        ...
        const complexity = (() => {
            ...
            if (this.objects.length >= this.maxObjects) {
                if (this.children.length !== 0)
                    throw new Error("Critical Logic Error");
                this.split();
