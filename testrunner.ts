...
if(
    JSON.stringify(qt.retrieve({x: 1, y: 1, width: 2, height: 2})) == 
    JSON.stringify([{x: 1, y: 1, width: 1, height: 1, reference: 'a'}, {x: 2, y: 2, width: 1, height: 1}, {x: 3, y: 3, width: 1, height: 1}, {x: 1.5, y: 2.6, width: 2, height: 2}])
) { console.log('quadtree test passed');
    // testrunner quadtree status header set to success
    var div = document.getElementById('testrunner-quadtree') as HTMLElement;
    div.style.backgroundColor = 'lightgreen';
    div.innerHTML = 'quadtree tests success';
} else { console.log('quadtree test failed');
...
{ // testrunner complete
// get testrunner div
var div = document.getElementById('testrunner') as HTMLElement;
